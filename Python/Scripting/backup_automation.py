...
try:
    ...
    if not src_file_name:
        print("The source file name should be provided")
        exit()

    try:
        if src_file_name and dst_file_name and src_dir and dist_dir:
            src_dir = src_dir+src_file_name
        ...
        shutil.copy2(src_dir, dst_r)

        print("Backup Successful")
    except FileNotFoundError:
        print("File does not exist,\ please provide the complete file path")

        # Backup for folders only
    except PermissionError:
        dst_dir = dst_dir+date_format+dst_file_name
        shutil.copytree(src_file_name, dst_dir)
take_backup(backup_automation.py)
